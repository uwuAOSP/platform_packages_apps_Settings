...
vec4 main(vec2 fragCoord) {
    vec4 background = uTex.eval(fragCoord);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        float mixFactor = smoothstep(radius, 0.0, distanceToPoint);

        color.rgb = mix(color.rgb, pointColor.rgb, mixFactor);
        color.a = mix(color.a, pointColor.a, mixFactor);
    }
    ...
    color.a = clamp(color.a + noiseValue * uAlphaOffset * 0.08, 0.0, 1.0);
    color.a *= uAlphaMulti;

    color.rgb += (10.0 / 255.0) * gradientNoise(fragCoord.xy) - (5.0 / 255.0);

    vec4 overlay = vec4(color.rgb * color.a, color.a);
    return overlay + background * (1.0 - overlay.a);
}
